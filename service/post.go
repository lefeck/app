...
type PostService interface {
	...
	AddComment(user *model.User, id string, comment *model.Comment) (*model.Comment, error)
...
func (p *postService) Get(user *model.User, id string) (*model.Post, error) {
	...
	err = p.postRepository.IncView(uint(pid))
	if err != nil {
		return nil, err
	}
...
func (p *postService) DelLike(user *model.User, id string) error {
	...
	return p.postRepository.DelLike(uint(pid), uint(user.ID))
}
...
func (p *postService) AddComment(user *model.User, id string, comment *model.Comment) (*model.Comment, error) {
	...
	comment = &model.Comment{PostID: uint(pid), UserID: user.ID}
